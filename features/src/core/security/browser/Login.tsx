...
const logo = require("../../layout/browser/assets/logo.png");
    ...
    useEffect(() => {
        if(location.pathname === "/token-callback") {
            const getToken = async () => {
                ...
                const tokenInfo = await authClient.code.getToken(document.location.href);
                    ...
                    render={({ handleSubmit, submitting, submitError }) => (
                        <form onSubmit={handleSubmit} style={{ height: "calc(100% - 57px)" }}>
                            ...
                            <GridHS container mt={1}>
                                <GridHS xs={12}>
                                    <Field<string> name={getPropertyName<Credentials>(x => x.username)} validate={validator().require().build()}>
                                        {({ input: { name, value, onChange, onBlur, onFocus }, meta: { error, touched } }) => (
                                            <>
                                                <label className="info-field" htmlFor={name}>Username</label>
                                                <TextField
                                                    autoFocus
                                ...
                                <GridHS xs={12}>
                                    <Field<string> name={getPropertyName<Credentials>(x => x.password)} validate={validator().require().build()}>
                                        {({ input: { name, value, onChange, onBlur, onFocus }, meta: { error, touched } }) => (
                                            <>
                                                <label className="info-field" htmlFor={name}>Password</label>
                                                <TextField
...
function createAuthClient(){
    return new ClientOAuth2({
        ...
        redirectUri: `${(document.location.protocol + '//' + document.location.host)}/token-callback`,
